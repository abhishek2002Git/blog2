...
public class RoundRobin {
  ...
  public static void main(String[] args) {
    ...
    //0
    while (timer < arrival[0]) timer++; //Incrementing Timer until the first process arrives
    queue[0] = 1;
    ...
    while (true) {
      ...
      for (int i = 0; (i < n) && (queue[i] != 0); i++) {
        ...
        if (
          (temp_burst[queue[0] - 1] == 0) && (complete[queue[0] - 1] == false)
        ) {
  ...
  public static void queueUpdation(
    int queue[],
    int timer,
    int arrival[],
    int n,
  ...
  public static void checkNewArrival(
    ...
    if (timer <= arrival[n - 1]) {
      ...
      if (newArrival) queueUpdation(queue, timer, arrival, n, maxProccessIndex); //adds the index of the arriving process(if any)
    }
  }
